...
export default {
  ...
  props: ["record", "value", "config", "parentDisabled", "dynamicData","dictOption"],
  data() {
    return {
      fileList: [],
  ...
  methods: {
        ...
        const arr = this.fileList.map(item => {
          if (typeof item.response !== "undefined") {
            ...
            return {
              ...
              name: item.name,
              status: item.status,
              uid: res.data?res.data.fileId || Date.now():Date.now(),
              url: res.message || ""
            };
          } else {
            return {
    ...
    handlePreview(file) {
      ...
      if (downloadWay === "a") {
        ...
        a.href =`${window._CONFIG['domianURL']}/`+file.url || file.thumbUrl;
