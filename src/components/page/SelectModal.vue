<template>
  <a-modal
    ...
    <template slot="footer">
      <a-button style="background-color: #ffffff;" @click="close">关闭</a-button>
      <a-button type="primary" @click="handleDepartment">确认</a-button>
    </template>
    ...
    <a-form>
      ...
      <a-form-item v-if="isMultiDepart" :labelCol="{span:4}" :wrapperCol="{span:20}" style="margin-bottom:10px" :validate-status="validate_status2">
        ...
        <a-select show-search :filterOption="filterOption" @change="handleDepartChange" :class="{'valid-error':validate_status2=='error'}" placeholder="请选择切换部门" style="margin-left:10px;width: 80%">
          <a-icon slot="suffixIcon" type="gold" />
