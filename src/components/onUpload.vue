  ...
  <div :id="containerId" style="position: relative">
    ...
    <a-upload
      ...
      <template>
        ...
        <a-button :style="error?'color: red;border-color: red;':''">
    ...
    <template v-if="fileList.length>0" >
      <template  v-for="(file,fileKey) of fileList">
        <div :key="fileKey" style="position: relative;">
          ...
          <a-tooltip v-else-if="file.status==='done'" :title="file.name">
            <span v-if="fileType === 'image'">
              <img class="j-editable-image" @click="filePop=true" :src="file.url" alt="无图片" />
            </span>
            <span v-else>
          ...
          <template style="width: 30px">
            <a-dropdown :trigger="['click']" placement="bottomRight"  style="margin-left: 10px;">
              ...
              <a-menu slot="overlay">
                <a-menu-item >
                  <span  @click="handleClickDownloadFile(file.url)"><a-icon type="download" />&nbsp;下载</span>
                </a-menu-item>
                <a-menu-item >
                  <span  @click="handleClickDelFile(fileKey)"><a-icon type="delete"/>&nbsp;删除</span>
                </a-menu-item>
                <a-menu-item>
                  <span @click="filePop=true"><a-icon type="bars"  /> 更多</span>
                </a-menu-item>
              </a-menu>
            </a-dropdown>
          </template>
    ...
    <a-modal :title="fileType === 'file' ? '文件上传':'图片上传'" :visible="filePop" :footer="null" @cancel="filePop=false">
      <j-upload
      :fileType="fileType"
      :buttonVisible="fileType === 'file' ? true:false"
      :number="1"
      :bizPath="bizPath"
      v-model="value"
      >
  ...
  export default {
    ...
    computed:{
      ...
      complistType(){
        ...
        return this.fileType === FILE_TYPE_IMG?'text':'text'
      }
    },
    created(){
