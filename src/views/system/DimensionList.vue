<template xmlns:background-color="http://www.w3.org/1999/xhtml">
  <a-row :gutter="10">
          ...
          <a-card :bordered="false" v-if="selectedKeys.length>0">
            <a-form :form="form">
              ...
              <a-form-item
                ...
                :wrapperCol="wrapperCol"
                label="简称">
                <a-textarea placeholder="请输入简称" v-decorator="['memo', {'initialValue':''}]"/>
              </a-form-item>
            </a-form>
    ...
    <dimensiont-modal ref="departModal" @ok="loadTree"></dimensiont-modal>
  </a-row>
</template>
<script>
  import DimensiontModal from './modules/DimensiontModal'
  import pick from 'lodash.pick'
  
  import {queryDimTreeList, searchByKeywords, deleteByDepartId} from '@/api/api'
    ...
    methods: {
      ...
      setValuesToForm(record1) {
        ...
        if(record.businessType){
          const businessType =Number(record.businessType)
          if(businessType&8){
            businessTypeList.push(8)
          }
          if(businessType&4){
            businessTypeList.push(4)
          }
          if(businessType&2){
            businessTypeList.push(2)
          }
          if(businessType&1){
            businessTypeList.push(1)
          }
          record.businessType=businessTypeList
        }else{
          record.businessType=[0]
        }
        console.log(record.businessType)
        this.$nextTick(() => {
          this.form.getFieldDecorator('fax', {initialValue: ''})

          this.form.setFieldsValue(pick(record, 'departName','orgCategory','businessType', 'orgCode', 'departOrder', 'mobile', 'fax', 'address', 'memo'))
        })
      },
