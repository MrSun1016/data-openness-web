<template>
  <div>
    <a-card :bordered="false" v-show="!isshowContent">
      <!-- 查询区域 -->
          ...
          <a-row :gutter="24">
            ...
            <a-col :span="6">
              <a-form-item label="状态">
                <a-input placeholder="请选择" v-model="queryParam.titile"></a-input>
              </a-form-item>
            </a-col>
            <a-col :span="6">
            ...
            <!-- <a-form-item label="状态">
            <a-select v-model="matchType" placeholder="请选择">
      ...
      <!-- 操作按钮区域 -->
      <div class="table-operator" style="line-height: 60px;margin-bottom: 0px;">
        <a-button @click="handleAdd" type="primary">发布公告</a-button>
      ...
      <!-- 表单区域 -->
      <sysAnnouncement-modal ref="modalForm" @ok="modalFormOk" @appointment="appointment"></sysAnnouncement-modal>
      <!-- 查看详情 -->
      <!-- <j-modal
        class="detail-modal"
        title="查看详情"
...
export default {
  ...
  methods: {
    ...
    releaseData: function(id) {
      var that = this
      if (this.issue == '预约发布') {
        doReleaseData({ id: id, appointment: this.appointment }).then(res => {
          if (res.success) {
    ...
    handleDetail: function(record) {
      ...
      this.$refs.content.getContent(record)
    }
  }
}
</script>
