            ...
            <a-timeline-item v-for="(item, index) in flowRecordList" :key="index" :color="setColor(item.finishTime)">
              ...
              <a-card :body-style="{ padding: '10px' }">
                <label v-if="item.assigneeName && item.finishTime" style="font-weight: normal; margin-right: 30px"
                  ...
                  <a-tag type="info" size="mini">{{ item.deptName }}</a-tag></label
                >
                <label v-if="item.candidate" style="font-weight: normal; margin-right: 30px"
                  >候选办理人： {{ item.candidate }}</label
                >
                <label style="font-weight: normal">接收时间： </label
                ><label  style="color: #8a909c; font-weight: normal">{{ item.createTime  | timeReverse}}</label>
                <label v-if="item.finishTime" style="margin-left: 30px; font-weight: normal">办结时间： </label
                ><label v-if="item.finishTime" style="color: #8a909c; font-weight: normal">{{ item.finishTime | timeReverse }}</label>
...
export default {
  ...
  methods: {
    ...
    getModelDetail() {
      // 发送请求，获取xml
      readXmlByDataId({ processCode: this.dataId }).then((res) => {
    ...
    // 流程进行情况
    getFlowViewer() {
      getFlowViewerByDataId({ processCode: this.dataId }).then((res) => {
    ...
    /** 流程流转记录 */
    getFlowRecordList() {
      const params = { processCode: this.dataId }
      flowRecord(params)
        .then((res) => {
          // console.log(res)
    ...
    fillColor() {
      ...
      modeler._definitions.rootElements[0].flowElements.forEach((n) => {
        ...
        if (n.$type === 'bpmn:UserTask') {
          ...
          //   })
          // }
          //  highlight-todo 黄色   highlight 绿色  highlight-todo 橙色  highlight-back红色
            if (completeTask) {
            canvas.addMarker(n.id, completeTask.completed ? 'highlight' : 'highlight-todo')
            canvas.addMarker(n.id, completeTask.back ? 'highlight-back' : 'highlight-noback')
            n.outgoing.forEach((nn) => {
              ...
              if (targetTask) {
                if (todoTask && completeTask.key === todoTask.key && !todoTask.completed) {
                  canvas.addMarker(nn.id, todoTask.completed ? 'highlight' : 'highlight-todo')
                  canvas.addMarker(nn.targetRef.id, todoTask.completed ? 'highlight-todo' :  'highlight')
                } else {
                  canvas.addMarker(nn.id, targetTask.completed ? 'highlight' : 'highlight-todo')
                  canvas.addMarker(nn.targetRef.id, targetTask.completed ? 'highlight' : 'highlight-todo')
                }
        ...
        // 并行网关
          ...
          console.log(n.outgoing,'n.outgoing4')
          n.outgoing.forEach((nn) => {
