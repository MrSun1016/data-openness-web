  ...
  <div>
    <el-tabs class="tabs" v-model="activeName" @tab-click="handleClick">
      <el-tab-pane label="数据预览" name="second">
        ...
        >
          <el-table-column width="180" v-for="item in serviceData.titleList" :label="item.title" :key="item.code">
            ...
            >
              ...
              <div class="source" id="container" style="margin: 0 24px">
                ...
                >
                  ...
                  <el-table-column prop="enName" label="源头表字段" min-width="90%" :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                      <i class="iconfont icon-yuechi" v-show="scope.row.primaryKey"></i>
                      <span style="margin-left: 8px">{{ scope.row.enName }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="chName" label="中文名称"  :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                      <span style="margin-left: 8px">{{ scope.row.name }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="dataType" label="类型" :show-overflow-tooltip="true"> </el-table-column>
                </el-table>
              </div>
              <!-- 目的表 -->
              <div class="source">
                ...
                >
                  ...
                  <el-table-column prop="enName" label="目的表字段" min-width="90%" :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                      <i class="iconfont icon-yuechi" v-show="scope.row.primaryKey"></i>
                      <span style="margin-left: 8px">{{ scope.row.enName }}</span>
                    </template>
                  ...
                  <el-table-column prop="chName" label="中文名称"  :show-overflow-tooltip="true">
                    <template slot-scope="scope">
                      <span style="margin-left: 8px">
                        {{ scope.row.fieldDescription ? scope.row.fieldDescription : scope.row.name  }}
                      </span>
                    </template>
                  </el-table-column>
                  <el-table-column prop="dataType" label="类型"  :show-overflow-tooltip="true"> </el-table-column>
  ...
  methods: {
    ...
    handleEnNameMap() {
      ...
      this.sourceTableData.forEach((item, index) => {
        let resIndex = goalTableDataCopy.findIndex((v) => v.enName.toLowerCase() === item.enName.toLowerCase())
        if (resIndex != -1) {
    ...
    onSubmit() {
      ...
      Promise.all([p1, p2]).then(() => {
        ...
        if (this.resourceData.autoCreateTableFlag === '1') {//自动创建目标表
          this.exchangeTaskBo.destTableName = this.resourceData.targetTabl
        } else {
          this.exchangeTaskBo.destTableName = this.resourceData.targetTable
        }
    ...
    onAssignment() {
      ...
      if (this.resourceData.autoCreateTableFlag === '1') {
        this.resourceDataReq = { ...this.resourceData, ...this.resourceData2 }
        this.resourceDataReq.targetTable = this.resourceData.targetTabl
      } else this.resourceDataReq = { ...this.resourceData, ...this.resourceData2 }
    },
    onCancel() {
      ;(this.SQLstatement = ''),
