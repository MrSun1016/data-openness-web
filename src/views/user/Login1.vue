<template>
  <div class="main">
    <a-tabs default-active-key="2" @change="handleTabClick">
      <!-- <a-tab-pane key="2" tab="集团用户">
        <a-form
          :form="form"
      ...
      <a-tab-pane key="2" tab="系统用户" force-render>
...
export default {
  ...
  methods: {
    ...
    getCaptcha(e) {
      ...
      this.form.validateFields(['mobile'], { force: true }, (err, values) => {
        if (!values.mobile) {
          ...
          smsParams.smsmode = '0'
          postAction('/dtwave-om/sys/sms', smsParams)
            .then(res => {
              if (!res.success) {
                setTimeout(hide, 0)
    ...
    handleChangeCheckCode() {
      this.currdatetime = new Date().getTime()
      getAction(`/dtwave-om/sys/randomImage/${this.currdatetime}`)
        .then(res => {
          if (res.success) {
